 ...
 * <p>
 * <b>WARNING:</b> While this class is public, it should not be treated as public API and its API may change in
 * incompatible ways between dot dot releases and even patch releases. You should not rely on this class even existing.
 *
 */
public class SwingUtilities2 {
	/**
	 * The {@code AppContext} key for our one {@code LAFState} instance.
	 */
	public static final Object LAF_STATE_KEY = new StringBuffer("LookAndFeel State");

	public static final Object MENU_SELECTION_MANAGER_LISTENER_KEY = new StringBuffer(
	...
	private static final int MAX_CHAR_INDEX = 'W' + 1;

	public static final FontRenderContext DEFAULT_FRC = new FontRenderContext(null, false, false);

	/**
	 * Attribute key for the content elements. If it is set on an element, the element is considered to be a line
	 * break.
	 */
	public static final String IMPLIED_CR = "CR";

	/**
	 * Used to tell a text component, being used as an editor for table or tree, how many clicks it took to start
	 * editing.
	 */
	 ...
	 * @param limit
	 *                limit
	 * @return {@code true} if TextLayout is required {@code false} if TextLayout is not required
	 */
	...
	/**
	 * Returns the left side bearing of the first character of string. The left side bearing is calculated from the
	 * passed in FontMetrics. If the passed in String is less than one character {@code 0} is returned.
	 *
	 * @param c
	 ...
	 *
	 * @return the left side bearing of the first character of string or {@code 0} if the string is empty
	 */
	...
	/**
	 * Returns the FontMetrics for the current Font of the passed in Graphics. This method is used when a Graphics
	 * is available, typically when painting. If a Graphics is not available the JComponent method of the same name
	 * should be used.
	 * <p>
	 * Callers should pass in a non-null JComponent, the exception to this is if a JComponent is not readily
	 * available at the time of painting.
	 * <p>
	...
	/**
	 * Returns the FontMetrics for the specified Font. This method is used when a Graphics is available, typically
	 * when painting. If a Graphics is not available the JComponent method of the same name should be used.
	 * <p>
	 * Callers should pass in a non-null JComonent, the exception to this is if a JComponent is not readily
	 * available at the time of painting.
	 * <p>
	 * This does not necessarily return the FontMetrics from the Graphics.
	 *
	...
	/**
	 * Returns the width of the passed in String. If the passed String is {@code null}, returns zero.
	 *
	 * @param c
	...
	/**
	 * Clips the passed in String to the space provided. NOTE: this assumes the string does not fit in the available
	 * space.
	 *
	 * @param c
	 *                JComponent that will display the string, may be null
	...
	public static String clipString(JComponent c, FontMetrics fm,
		...
		String s = string;
		String clipString = "...";
		int availableTextWidth = availTextWidth - SwingUtilities2.stringWidth(c, fm, clipString);
		...
		synchronized(charsBufferLock) {
			int stringLength = syncCharsBuffer(s);
		...
		if(needsTextLayout) {
			...
			s = s.substring(0, measurer.nextOffset(availableTextWidth));
	...
	public static void drawString(JComponent c, Graphics g, String text,
		...
		if(isPrinting(g)) {
			...
			if(g2d != null) {
				/*
				 * The printed text must scale linearly with the UI. Calculate the width on screen,
				 * obtain a TextLayout with advances for the printer graphics FRC, and then justify it
				 * to fit in the screen width. This distributes the spacing more evenly than directly
				 * laying out to the screen advances.
				 */
				String trimmedText = text.stripTrailing();
				if(!trimmedText.isEmpty()) {
					float screenWidth = (float) g2d.getFont()
							.getStringBounds(trimmedText, getFontRenderContext(c))
							.getWidth();
	...
	/**
	 * Returns true if the given point is within the actual bounds of the JList item at index (not just inside the
	 * cell).
	 */
	 ...
	 * (Column must be 0). Does not check the "Table.isFileList" property. That should be checked before calling
	 * this method. This is used to make Windows {@literal L&F} JFileChooser act like native dialogs.
	 */
	...
	/**
	 * Request focus on the given component if it doesn't already have it and {@code isRequestFocusEnabled()}
	 * returns true.
	 */
	...
	public static float getFontCharsWidth(char[] data, int offset, int len,
		...
		if(useFPAPI) {
			Rectangle2D bounds = fm.getFont().getStringBounds(data, offset, offset + len,
	...
	/**
	 * This method should be used for drawing a borders over a filled rectangle. Draws vertical line, using the
	 * current color, between the points {@code
	 * (x, y1)} and {@code (x, y2)} in graphics context's coordinate system. Note: it use
	 * {@code Graphics.fillRect()} internally.
	 *
	 * @param g
	...
	public static void drawVLine(Graphics g, int x, int y1, int y2) {
		...
		g.fillRect(x, py1, 1, py2 - py1 + 1);
	}

	/**
	 * This method should be used for drawing a borders over a filled rectangle. Draws horizontal line, using the
	 * current color, between the points {@code
	 * (x1, y)} and {@code (x2, y)} in graphics context's coordinate system. Note: it use
	 * {@code Graphics.fillRect()} internally.
	 *
	 * @param g
	 *                Graphics to draw the line to.
	...
	public static void drawHLine(Graphics g, int x1, int x2, int y) {
		...
		g.fillRect(px1, y, px2 - px1 + 1, 1);
	}

	/**
	 * This method should be used for drawing a borders over a filled rectangle. Draws the outline of the specified
	 * rectangle. The left and right edges of the rectangle are at {@code x} and {@code x + w}. The top and bottom
	 * edges are at {@code y} and {@code y + h}. The rectangle is drawn using the graphics context's current color.
	 * Note: it use {@code
	...
	/*
	 * Checks if two given FontRenderContexts are compatible for printing. We can't just use equals as we want to
	 * exclude from the comparison : + whether AA is set as irrelevant for printing and shouldn't affect printed
	...
	private static boolean isFontRenderContextPrintCompatible(FontRenderContext frc1,
		...
		/*
		 * That's the end of the cheap tests, need to get and compare the transform matrices. We don't care
		 * about the translation components, so return true if they are otherwise identical.
		 */
	...
	/*
	 * Tries it best to get Graphics2D out of the given Graphics returns null if can not derive it.
	 */
	...
	/*
	 * Returns FontRenderContext associated with Component. FontRenderContext from Component.getFontMetrics is
	...
	/**
	 * A convenience method to get FontRenderContext. Returns the FontRenderContext for the passed in FontMetrics or
	...
	/**
	 * Determines whether the SelectedTextColor should be used for painting text foreground for the specified
	 * highlight.
	 *
	...
	/**
	 * LSBCacheEntry is used to cache the left side bearing (lsb) for a particular {@code Font} and
	 * {@code FontRenderContext}. This only caches characters that fall in the range {@code MIN_CHAR_INDEX} to
	 * {@code MAX_CHAR_INDEX}.
	 */
	private static class LSBCacheEntry {
		...
		public int getLeftSideBearing(char aChar) {
			...
			if(lsb == UNSET) {
				...
				if(lsb < 0) {
					/*
					 * HRGB/HBGR LCD glyph images will always have a pixel on the left used in
					 * colour fringe reduction. Text rendering positions this correctly but here we
					 * are using the glyph image to adjust that position so must account for it.
					 */
	 ...
	 * {@code UIResource} for the specified image file name. The image is loaded using {@code getResourceAsStream},
	 * starting with a call to that method on the base class parameter. If it cannot be found, searching will
	 * continue through the base class' inheritance hierarchy, up to and including {@code rootClass}.
	 *
	 * @param baseClass
	...
	private static byte[] getIconBytes(final Class<?> baseClass, final Class<?> rootClass, final String imageFile) {
		/*
		 * Copy resource into a byte array. This is necessary because several browsers consider
		 * Class.getResource a security risk because it can be used to load additional classes.
		 * Class.getResourceAsStream just returns raw bytes, which we can convert to an image.
		 */
	...
	/**
	 * Returns an integer from the defaults table. If {@code key} does not map to a valid {@code Integer}, or can
	 * not be convered from a {@code String} to an integer, the value 0 is returned.
	 *
	 * @param key
	 *                an {@code Object} specifying the int.
	 * @return the int
	 */
	...
	/**
	 * Returns an integer from the defaults table that is appropriate for the given locale. If {@code key} does not
	 * map to a valid {@code Integer}, or can not be convered from a {@code String} to an integer, the value 0 is
	 * returned.
	 *
	 * @param key
	 *                an {@code Object} specifying the int. Returned value is 0 if {@code key} is not available,
	 * @param l
	 *                the {@code Locale} for which the int is desired
	 * @return the int
	 */
	...
	/**
	 * Returns an integer from the defaults table. If {@code key} does not map to a valid {@code Integer}, or can
	 * not be convered from a {@code String} to an integer, {@code default} is returned.
	 *
	 * @param key
	 *                an {@code Object} specifying the int. Returned value is 0 if {@code key} is not available,
	 * @param defaultValue
	 *                Returned value if {@code key} is not available, or is not an Integer
	 * @return the int
	 */
	...
	/**
	 * Returns an integer from the defaults table that is appropriate for the given locale. If {@code key} does not
	 * map to a valid {@code Integer}, or can not be convered from a {@code String} to an integer, {@code default}
	 * is returned.
	 *
	 * @param key
	 *                an {@code Object} specifying the int. Returned value is 0 if {@code key} is not available,
	...
	/**
	 * Change focus to the visible component in {@code JTabbedPane}. This is not a general-purpose method and is
	 * here only to permit sharing code.
	 */
	@SuppressWarnings("deprecation")
	...
	/**
	 * Submits a value-returning task for execution on the EDT and returns a Future representing the pending results
	 * of the task.
	 *
	 * @param task
	 *                the task to submit
	 ...
	 * @return a Future representing pending completion of the task, and whose {@code get()} method will return the
	 *         given result value upon completion
	 * @throws NullPointerException
	 *                 if the task is null
	 */
	...
	/**
	 * Sets the {@code SKIP_CLICK_COUNT} client property on the component if it is an instance of
	 * {@code JTextComponent} with a {@code DefaultCaret}. This property, used for text components acting as editors
	 * in a table or tree, tells {@code DefaultCaret} how many clicks to skip before starting selection.
	 */
	...
	/**
	 * Return the MouseEvent's click count, possibly reduced by the value of the component's
	 * {@code SKIP_CLICK_COUNT} client property. Clears the {@code SKIP_CLICK_COUNT} property if the mouse event's
	 * click count is 1. In order for clearing of the property to work correctly, there must be a mousePressed
	 * implementation on the caller with this call as the first line.
	 */
	...
	/**
	 * Used by the {@code liesIn} method to return which section the point lies in.
	 *
	 * @see #liesIn
	 */
	public enum Section {
	...
	/**
	 * This method divides a rectangle into two or three sections along the specified axis and determines which
	 * section the given point lies in on that axis; used by drag and drop when calculating drop locations.
	 * <p>
	 * For two sections, the rectangle is divided equally and the method returns whether the point lies in
	 * {@code Section.LEADING} or {@code Section.TRAILING}. For horizontal divisions, the calculation respects
	 * component orientation.
	 * <p>
	 * For three sections, if the rectangle is greater than or equal to 30 pixels in length along the axis, the
	 * calculation gives 10 pixels to each of the leading and trailing sections and the remainder to the middle. For
	 * smaller sizes, the rectangle is divided equally into three sections.
	 * <p>
	 * Note: This method assumes that the point is within the bounds of the given rectangle on the specified axis.
	 * However, in cases where it isn't, the results still have meaning: {@code Section.MIDDLE} remains the same,
	 * {@code Section.LEADING} indicates that the point is in or somewhere before the leading section, and
	 * {@code Section.TRAILING} indicates that the point is in or somewhere after the trailing section.
	 *
	 * @param rect
	 *                the rectangle
	 * @param p
	 *                the point the check
	 * @param horizontal
	 *                {@code true} to use the horizontal axis, or {@code false} for the vertical axis
	 * @param ltr
	 *                {@code true} for left to right orientation, or {@code false} for right to left orientation;
	 *                only used for horizontal calculations
	 * @param three
	 *                {@code true} for three sections, or {@code false} for two
	 *
	...
	/**
	 * This method divides a rectangle into two or three sections along the horizontal axis and determines which
	 * section the given point lies in; used by drag and drop when calculating drop locations.
	 * <p>
	 * See the documentation for {@link #liesIn} for more information on how the section is calculated.
	 *
	 * @param rect
	 *                the rectangle
	 * @param p
	...
	/**
	 * This method divides a rectangle into two or three sections along the vertical axis and determines which
	 * section the given point lies in; used by drag and drop when calculating drop locations.
	 * <p>
	 * See the documentation for {@link #liesIn} for more information on how the section is calculated.
	 *
	...
	/**
	 * Maps the index of the column in the view at {@code viewColumnIndex} to the index of the column in the table
	 * model. Returns the index of the corresponding column in the model. If {@code viewColumnIndex} is less than
	 * zero, returns {@code viewColumnIndex}.
	 *
	 * @param cm
	 *                the table model
	...
	/**
	 * Maps the index of the column in the {@code cm} at {@code modelColumnIndex} to the index of the column in the
	 * view. Returns the index of the corresponding column in the view; returns {@code -1} if this column is not
	 * being displayed. If {@code modelColumnIndex} is less than zero, returns {@code modelColumnIndex}.
	 *
	 * @param cm
	 *                the table model
	...
	/**
	 * Sets the antialiasing rendering hint if its value is not null. Null hint value indicates that the passed
	 * graphics is not instance of Graphics2D.
	 *
	 * @param g
	 *                the graphics
	 * @param hint
	 ...
	 *                the component
	 * @param key
	 *                an {@code String} specifying the key for the desired boolean value
	 * @return the boolean value of the client property if it is set or the {@L&F} property in other case.
	 */
	...
	/**
	 * Returns whether or not the scale used by {@code GraphicsConfiguration} was changed.
	 *
	 * @param ev
