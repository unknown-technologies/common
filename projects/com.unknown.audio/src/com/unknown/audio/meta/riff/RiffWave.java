...
public class RiffWave extends Riff {
	...
	public int getSampleCount() {
		...
		return rawSamples.length / nch / (bits / 8);
	}
	...
	public byte[] get8bitSamples() {
		if(getBitsPerSample() != 8 || getSampleFormat() != WaveFormatChunk.WAVE_FORMAT_PCM) {
			throw new IllegalStateException(
					"cannot get 8bit samples for " + getBitsPerSample() + "bit sample data");
		}
		// convert unsigned to signed
		byte[] rawSamples = getData().getData();
		byte[] samples = new byte[rawSamples.length];
		for(int i = 0; i < samples.length; i++) {
			samples[i] = (byte) (rawSamples[i] - 128);
		}
		return samples;
	}

	public void set8bitSamples(byte[] samples) {
		// convert signed to unsigned
		byte[] rawSamples = new byte[samples.length];
		for(int i = 0; i < samples.length; i++) {
			rawSamples[i] = (byte) (samples[i] + 128);
		}
	...
	public void set16bitSamples(float[] samples) {
		if(getBitsPerSample() != 16) {
			throw new IllegalStateException(
					"cannot set 16bit samples for " + getBitsPerSample() + "bit sample data");
		}
		byte[] rawSamples = new byte[samples.length * 2];
		for(int i = 0; i < samples.length; i++) {
		...
		if(bits == 8) {
			...
			for(int ch = 0; ch < nch; ch++) {
				for(int i = 0; i < samples[ch].length; i++) {
					int off = i * nch + ch;
					samples[ch][i] = (rawSamples[off] - 128) << 24;
				}
			}
			return samples;
		} else if(bits == 16) {
	...
	public float[][] getFloatSamples() {
		...
		if(bits == 8) {
			...
			for(int ch = 0; ch < nch; ch++) {
				for(int i = 0; i < samples[ch].length; i++) {
					int off = i * nch + ch;
					samples[ch][i] = ((byte) (rawSamples[off] - 128)) / 128.0f;
				}
			}
			return samples;
		} else if(bits == 16) {
