...
public class BRSTM implements Stream {
	...
	public BRSTM(DataFile file) throws FileFormatException, IOException {
		this.file = file;
	...
	public int read_16bitBE(long offset) throws IOException {
		...
		file.read(data);
		return Endianess.get16bitBEu(data);
	}
	...
	public long read_32bitBE(long offset) throws IOException {
		...
		file.read(data);
		return Endianess.get32bitBE(data);
	}
	...
	private void readHeader() throws FileFormatException, IOException {
		...
		if(this.codec == CODEC_ADPCM) {
			...
			for(int j = 0; j < this.channel_count; j++) {
				for(int i = 0; i < 16; i++) {
					this.coef[j][i] = (short) read_16bitBE(
							head_offset + coef_offset + j * coef_spacing + i * 2);
				}
		...
		for(int i = 0; i < this.channel_count; i++) {
			this.channel_start_offset[i] = this.channel_offset[i] = this.start_offset +
					i * this.interleave_block_size;
		}
	...
	private int[] doDecode() throws IOException {
		long start = current_byte;
		if((loop_flag != 0) && (current_byte >= loop_end_offset)) {
			start = loop_start_offset;
		}
		long blocks = start / interleave_block_size;
		if((loop_flag != 0) && (current_byte >= loop_end_offset)) {
		...
		start = start - (blocks * interleave_block_size);
		start = (start / 8) * 8;
		int startsample = (int) (start / 8.0 * 14.0);
	...
	public String toString() {
		return "BRSTM[" + sample_rate + "Hz,16bit," + sample_count + " samples,loop:" +
