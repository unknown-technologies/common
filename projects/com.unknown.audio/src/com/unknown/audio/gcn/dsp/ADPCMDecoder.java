...
public class ADPCMDecoder {
	...
	public int[] decode_ngc_dsp(int offset, long first_sample, int samples_to_do, byte[] mem) {
		...
		long first = first_sample % 14;

		int[] samples = new int[samples_to_do * channelspacing];

		for(i = first, sample_count = 0; i < (first + samples_to_do); i++, sample_count += channelspacing) {
			int sample_byte = mem[offset + framesin * 8 + 1 + (int) (i / 2)] & 0xFF;

			samples[sample_count] = clamp16(((((((i & 1) != 0) ? get_low_nibble_signed(sample_byte)
					: get_high_nibble_signed(sample_byte)) * scale) << 11) + 1024 +
	...
	public int[] decode_ngc_dsp_alt(int offset, long first_sample, int samples_to_do, int ch, int nch, byte[] mem) {
		long i = first_sample;
		int sample_count;
		int channelspacing = 1;
