...
public class BFSTM implements Stream {
	...
	public BFSTM(DataFile file) throws FileFormatException, IOException {
		this.file = file;
	...
	private void readHeader() throws FileFormatException, IOException {
		...
		if(this.codec == CODEC_ADPCM) {
			...
			for(int j = 0; j < this.channel_count; j++) {
				...
				for(int i = 0; i < 16; i++) {
					this.coef[j][i] = (short) read_16bitBE(coef_offset + i * 2);
				}
	...
	public String toString() {
		return "BFSTM[" + sample_rate + "Hz,16bit," + sample_count + " samples,loop:" +
