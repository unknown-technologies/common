...
public class MultichannelDelay extends Effect {
	...
	public MultichannelDelay(int sampleRate, int channels, float[] times) {
	...
	protected float[] process(float[] input) {
		...
		for(int i = 0; i < channels; i++) {
			...
			history[i][write[i]++] = input[i] * (1.0f - feedback[i]) + feedback[i] * out[i];
			read[i] %= history[i].length;
