...
public class MemoryAllocator {
	...
	private static class Block {
		...
		boolean contains(long address) {
			if(size == 0xffffffffffffffffL) { // avoid overflow
				...
				return Long.compareUnsigned(address, base) >= 0 &&
						Long.compareUnsigned(address, base + size) < 0;
			}
		}
	...
	private Block find(long addr) {
		for(Block b = memory; b != null; b = b.nextBlock) {
			assert (b.nextBlock == null) || (b.nextBlock.base == b.base + b.size) : String
					.format("0x%x vs 0x%x", b.base + b.size, b.nextBlock.base);
	...
	private static void check(Block b) {
		...
		assert !b.free || b.prev == null || Long.compareUnsigned(b.prev.base, b.base) < 0 : String
				.format("prev=0x%016x, this=0x%016x", b.prev.base, b.base);
		assert !b.free || b.next == null || Long.compareUnsigned(b.base, b.next.base) < 0 : String
				.format("this=0x%016x, next=0x%016x", b.base, b.next.base);
		assert !b.free || b.prev == null || b.prev.next == b : String
				.format("this=0x%016x, this.prev.next=0x%016x", b.base, b.prev.next.base);
		assert !b.free || b.next == null || b.next.prev == b : String
				.format("this=0x%016x, this.next.prev=0x%016x", b.base, b.next.prev.base);
		assert b.prevBlock == null || Long.compareUnsigned(b.prevBlock.base, b.base) < 0 : String
				.format("prev=0x%016x, this=0x%016x", b.prevBlock.base, b.base);
		assert b.nextBlock == null || Long.compareUnsigned(b.base, b.nextBlock.base) < 0 : String
				.format("this=0x%016x, next=0x%016x", b.base, b.nextBlock.base);
		assert b.prevBlock == null || b.prevBlock.nextBlock == b : String.format(
				"this=0x%016x, this.prevBlock.nextBlock=0x%016x", b.base, b.prevBlock.nextBlock.base);
		assert b.nextBlock == null || b.nextBlock.prevBlock == b : String.format(
				"this=0x%016x, this.nextBlock.prevBlock=0x%016x", b.base, b.nextBlock.prevBlock.base);
	}
	...
	public synchronized void check() {
		...
		__assert((usedmem + freemem) == memorySize, String
				.format("0x%dx bytes in blocks vs 0x%dx memory region", usedmem + freemem, memorySize));
	}
}
