...
import static com.unknown.util.ui.plaf.SwingUtilities2.BASICMENUITEMUI_MAX_TEXT_OFFSET;
...
public class MenuItemLayoutHelper {
	...
	protected void reset(JMenuItem mi, Icon checkIcon, Icon arrowIcon, Rectangle viewRect, int gap,
			String accDelimiter, boolean isLeftToRight, Font font, Font accFont, boolean useCheckAndArrow,
			String propertyPrefix) {
		this.mi = mi;
	...
	private static int getMinTextOffset(String propertyPrefix) {
	...
	protected void calcWidthsAndHeights() {
		...
		if(isColumnLayout) {
			...
			SwingUtilities.layoutCompoundLabel(mi, fm, text, icon, verticalAlignment, horizontalAlignment,
					verticalTextPosition, horizontalTextPosition, viewRect, iconRect, textRect,
					gap);
	 ...
	 *
	 * @param propertyName
	 *                name of the property, which stores the maximal value.
	 * @param value
	 *                a value which pretends to be maximal
	...
	/**
	 * Answers should we do column layout for a menu item or not. We do it when a user doesn't set any alignments
	...
	/**
	 * Calculates maximal text offset. It is required for some L&Fs (ex: Vista L&F). The offset is meaningful only
	 * for L2R column layout.
	 *
	 * @param rect
	 *                the rectangle, the maximal text offset will be calculated for.
	 */
	private void calcMaxTextOffset(Rectangle rect) {
		if(!isColumnLayout || !isLeftToRight) {
			return;
		}

		// Calculate the current text offset
		int offset = rect.x + leadingGap + checkSize.maxWidth + afterCheckIconGap + iconSize.maxWidth + gap;
	 ...
	 *
	 * If horizontalAlignment, verticalTextPosition and horizontalTextPosition are default (user doesn't set any
	 * manually) the layouting algorithm is: Elements are layouted in the five columns: check icon + icon + text +
	 * accelerator text + arrow icon
	 *
	 * In the other case elements are layouted in the four columns: check icon + label + accelerator text + arrow
	 * icon Label is union of icon and text.
	 *
	...
	/**
	 * Fixes vertical alignment of all menu item elements if rect.y or (rect.y + rect.height) is out of viewRect
	 * bounds
	 */
	...
	private static void alignRect(Rectangle rect, int alignment, int origWidth) {
	...
	protected void layoutIconAndTextInLabelRect(LayoutResult lr) {
		...
		SwingUtilities.layoutCompoundLabel(mi, fm, text, icon, verticalAlignment, horizontalAlignment,
				verticalTextPosition, horizontalTextPosition, lr.labelRect, lr.iconRect, lr.textRect,
				gap);
	}

	private static void calcXPositionsLTR(int startXPos, int leadingGap, int gap, Rectangle... rects) {
	...
	private static void calcXPositionsRTL(int startXPos, int leadingGap, int gap, Rectangle... rects) {
		int curXPos = startXPos - leadingGap;
		for(Rectangle rect : rects) {
	 ...
	 *
	 * @param menuItem
	 *                the menu item whose parent will be returned.
	 ...
	 *
	 * @param values
	 *                array where the search will be performed.
	...
	public static class ColumnAlignment {
		...
		public static final ColumnAlignment LEFT_ALIGNMENT = new ColumnAlignment(
				SwingConstants.LEFT,
				SwingConstants.LEFT,
				SwingConstants.LEFT,
				SwingConstants.LEFT,
		...
		public static final ColumnAlignment RIGHT_ALIGNMENT = new ColumnAlignment(
