...
public abstract class ShellFolder extends File {
	...
	/**
	 * This method must be implemented to make sure that no instances of {@code ShellFolder} are ever serialized. If
	 * {@code isFileSystem()} returns {@code true}, then the object should be representable with an instance of
	 * {@code java.io.File} instead. If not, then the object is most likely depending on some internal (native)
	 * state and cannot be serialized.
	 *
	 * @return a java.io.File replacement object, or null if no suitable replacement can be found.
	 */
	@Serial
	protected abstract Object writeReplace() throws java.io.ObjectStreamException;

	/**
	 * Returns the path for this object's parent, or {@code null} if this object does not name a parent folder.
	 *
	 * @return the path as a String for this object's parent, or {@code null} if this object does not name a parent
	 *         folder
	 *
	...
	/**
	 * Returns a File object representing this object's parent, or {@code null} if this object does not name a
	 * parent folder.
	 *
	 * @return a File object representing this object's parent, or {@code null} if this object does not name a
	 *         parent folder
	 *
	...
	/**
	 * @param getLargeIcon
	 *                whether to return large icon (ignored in base implementation)
	...
	static {
		...
		try {
			shellFolderManager = (ShellFolderManager) managerClass.getDeclaredConstructor().newInstance();
	...
	/**
	 * Does {@code dir} represent a "computer" such as a node on the network, or "My Computer" on the desktop.
	 */
	...
	/**
	 * Returns true if folder allows creation of children. True for the "Desktop" folder, but false for the "My
	 * Computer" folder.
	 */
	@Override
	...
	public Object getFolderColumnValue(@SuppressWarnings("unused") int column) {
		return null;
	}

	/**
	 * Invokes the {@code task} which doesn't throw checked exceptions from its {@code call} method. If invocation
	 * is interrupted then Thread.currentThread().isInterrupted() will be set and result will be {@code null}
	 */
	...
	/**
	 * Invokes the {@code task} which throws checked exceptions from its {@code call} method. If invocation is
	 * interrupted then Thread.currentThread().isInterrupted() will be set and InterruptedException will be thrown
	 * as well.
	 */
	...
	public static interface Invoker {
		 ...
		 *                a task to invoke
		 * @throws Exception
		 *                 {@code InterruptedException} or an exception that was thrown from the {@code task}
