...
public class ImageCache {
	...
	private Entry getEntry(GraphicsConfiguration config, int w, int h, Object[] args) {
		Entry entry;
		...
		entries.addFirst(new SoftReference<>(entry));
		return entry;
	}
	...
	public Image getImage(GraphicsConfiguration config, int w, int h, Object[] args) {
		Entry entry = getEntry(config, w, h, args);
	...
	private static class Entry {
		...
		public String toString() {
			String value = super.toString() + "[ graphicsConfig=" + config + ", image=" + image + ", w=" +
					w + ", h=" + h;
			if(args != null) {
		...
		public boolean equals(GraphicsConfiguration config, int w, int h, Object[] args) {
			if(this.w == w && this.h == h && ((this.config != null && this.config.equals(config)) ||
