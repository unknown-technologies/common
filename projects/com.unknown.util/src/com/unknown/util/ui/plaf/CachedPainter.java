 ...
 * <ol>
 * <li>Invoke <code>paint</code> when you want to paint the image, if you are implementing <code>Icon</code> you'll
 * invoke this from <code>paintIcon</code>. The args argument is useful when additional state is needed.
 * <li>Override <code>paintToImage</code> to render the image. The code that lives here is equivalent to what previously
 * would go in <code>paintIcon</code>, for an <code>Icon</code>.
 * </ol>
 * The two ways to use this class are:
 * <ol>
 * <li>Invoke <code>paint</code> to draw the cached representation at the specified location.
 * <li>Invoke <code>getImage</code> to get the cached representation and draw the image yourself. This is primarily
 * useful when you are not using <code>VolatileImage</code>.
 * </ol>
 *
 *
 */
public abstract class CachedPainter {
	...
	/**
	 * Creates an instance of <code>CachedPainter</code> that will cache up to <code>cacheCount</code> images of
	 * this class.
	 *
	 * @param cacheCount
	...
	/**
	 * Renders the cached image to the passed in <code>Graphic</code>. If there is no cached image
	 * <code>paintToImage</code> will be invoked. <code>paintImage</code> is invoked to paint the cached image.
	 *
	 * @param c
	 *                Component rendering to, this may be null.
	 * @param g
	 *                Graphics to paint to
	 * @param x
	 *                X-coordinate to render to
	 * @param y
	 *                Y-coordinate to render to
	 * @param w
	 *                Width to render in
	 * @param h
	 *                Height to render in
	 * @param args
	 *                Variable arguments that will be passed to paintToImage
	 */
	...
	private Image getImage(Object key, Component c, int baseWidth, int baseHeight, int width, int height,
			Object... args) {
		int w = width;
		int h = height;
		GraphicsConfiguration config = getGraphicsConfiguration(c);
		ImageCache cache = getCache(key);
		Image image = cache.getImage(config, w, h, args);
		int attempts = 0;
		VolatileImage volatileImage = (image instanceof VolatileImage) ? (VolatileImage) image : null;
		do {
			boolean draw = false;
			...
			if(image == null) {
				...
				cache.setImage(config, w, h, args, image);
				draw = true;
				volatileImage = (image instanceof VolatileImage) ? (VolatileImage) image : null;
			}
			if(draw) {
				// Render to the Image
				...
				if(volatileImage == null) {
					...
					// SurfaceData sd =
					// SurfaceManager.getManager(volatileImage).getPrimarySurfaceData();
					double sx = 1; // sd.getDefaultScaleX();
					double sy = 1; // sd.getDefaultScaleY();
					if(Double.compare(sx, 1) != 0 || Double.compare(sy, 1) != 0) {
						g2.scale(1 / sx, 1 / sy);
					}
					paintToImage(c, image, g2, (int) Math.ceil(w * sx), (int) Math.ceil(h * sy),
							args);
				}
				g2.dispose();
			}
	...
	/**
	 * Creates the image to cache. This returns an opaque image, subclasses that require translucency or
	 * transparency will need to override this method.
	 *
	 * @param c
	 *                Component painting to
	 * @param w
	...
	private static GraphicsConfiguration getGraphicsConfiguration(Component c) {
		if(c == null) {
			return null;
		}
	...
	class PainterMultiResolutionCachedImage extends AbstractMultiResolutionImage {
		...
		public Image getResolutionVariant(double destWidth, double destHeight) {
			...
			return getImage(PainterMultiResolutionCachedImage.class, c, baseWidth, baseHeight, w, h, args);
		}
