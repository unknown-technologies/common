...
/**
 * An implementation of <code>LayoutStyle</code> that returns 6 for related components, otherwise 12. This class also
...
public class DefaultLayoutStyle extends LayoutStyle {
	...
	/**
	 * For some look and feels check boxes and radio buttons typically don't paint the border, yet they have padding
	 * for a border. Look and feel guidelines generally don't include this space. Use this method to subtract this
	 * space from the specified components.
	 *
	 * @param source
	 *                First component
	 * @param target
	...
	protected int getButtonGap(JComponent source, JComponent target, int position, int offset) {
		int off = offset;
		off -= getButtonGap(source, position);
		if(off > 0) {
			off -= getButtonGap(target, flipDirection(position));
		}
		if(off < 0) {
			return 0;
		}
	...
	/**
	 * If <code>c</code> is a check box or radio button, and the border is not painted this returns the inset along
	 * the specified axis.
	 */
	...
	/**
	 * Returns the amount to indent the specified component if it's a JCheckBox or JRadioButton. If the component is
	 * not a JCheckBox or JRadioButton, 0 will be returned.
	 */
	...
	private static Icon getIcon(AbstractButton button) {
	...
	private static boolean isLeftAligned(AbstractButton button, int position) {
	...
	private static boolean isRightAligned(AbstractButton button, int position) {
	...
	private static int getInset(JComponent c, int position) {
		return getInset(c.getInsets(), position);
	}

	private static int getInset(Insets insets, int position) {
		if(insets == null) {
			return 0;
		}
		switch(position) {
