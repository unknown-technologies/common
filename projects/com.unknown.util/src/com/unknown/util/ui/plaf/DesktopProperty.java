...
public class DesktopProperty implements UIDefaults.ActiveValue {
	...
	/**
	 * Invalidates the current value so that the next invocation of <code>createValue</code> will ask for the
	 * property again.
	 */
	 ...
	 * {@literal look&feel}. This update occurs by uninstalling and re-installing the UI objects. Requests are
	 * batched and collapsed into a single update pass because often many desktop properties will change at once.
	 */
	 ...
	 */
	protected Object configureValue(Object val) {
		Object result = val;
		if(result != null) {
			if(result instanceof Color) {
				return new ColorUIResource((Color) result);
			} else if(result instanceof Font) {
				return new FontUIResource((Font) result);
			} else if(result instanceof UIDefaults.LazyValue) {
				result = ((UIDefaults.LazyValue) result).createValue(null);
			} else if(result instanceof UIDefaults.ActiveValue) {
				result = ((UIDefaults.ActiveValue) result).createValue(null);
			}
		}
		return result;
	}
	...
	/**
	 * As there is typically only one Toolkit, the PropertyChangeListener is handled via a WeakReference so as not
	 * to pin down the DesktopProperty.
	 */
