...
/**
 * <b>WARNING:</b> This class is an implementation detail and is only public so that it can be used by two packages. You
 * should NOT consider this public API.
 * <p>
 * This component is intended to be used in a subclass of javax.swing.plaf.basic.BasicFileChooserUI. It relies heavily
...
public class FilePane extends JPanel implements PropertyChangeListener {
	...
	private final KeyListener detailsKeyListener = new KeyAdapter() {
		...
		/**
		 * Moves the keyboard focus to the first element whose prefix matches the sequence of alphanumeric keys
		 * pressed by the user with delay less than value of <code>timeFactor</code>. Subsequent same key
		 * presses move the keyboard focus to the next object that starts with the same letter until another key
		 * is pressed, then it is treated as the prefix with appropriate number of the same letters followed by
		...
		private int getNextMatch(int startIndex, int finishIndex) {
			...
			DetailsTableRowSorter sorter = getRowSorter();

			String prefix = typedString.toString().toLowerCase();

			// Search element
			for(int index = startIndex; index <= finishIndex; index++) {
				File file = (File) model.getElementAt(sorter.convertRowIndexToModel(index));
	...
	public void setViewType(int viewType) {
		...
		if(currentViewPanel != null) {
			Component owner = DefaultKeyboardFocusManager.getCurrentKeyboardFocusManager()
	...
	public Action getViewTypeAction(int type) {
		return new ViewTypeAction(type);
	}
	...
	protected void installDefaults() {
		...
		viewTypeActionNames[VIEWTYPE_DETAILS] = UIManager.getString("FileChooser.detailsViewActionLabelText",
				l);
	...
	/**
	 * Fetches the command list for the FilePane. These commands are useful for binding to events, such as in a
	 * keymap.
	 *
	...
	private void updateListRowCount(JList<?> lst) {
		if(smallIconsView) {
			lst.setVisibleRowCount(getModel().getSize() / 3);
		} else {
	...
	public JPanel createList() {
		...
		final JList<Object> lst = new JList<>() {
			@Override
		...
		lst.setCellRenderer(new FileRenderer());
		lst.setLayoutOrientation(JList.VERTICAL_WRAP);

		// 4835633 : tell BasicListUI that this is a file list
		lst.putClientProperty("List.isFileList", Boolean.TRUE);

		if(listViewWindowsStyle) {
			lst.addFocusListener(repaintListener);
		}

		updateListRowCount(lst);
		...
		lst.setModel(new SortableListModel());

		lst.addListSelectionListener(createListSelectionListener());
		lst.addMouseListener(getMouseHandler());

		JScrollPane scrollpane = new JScrollPane(lst);
		if(listViewBackground != null) {
			lst.setBackground(listViewBackground);
		}
		if(listViewBorder != null) {
			scrollpane.setBorder(listViewBorder);
		}

		lst.putClientProperty(AccessibleContext.ACCESSIBLE_NAME_PROPERTY, filesListAccessibleName);
	...
	class DetailsTableModel extends AbstractTableModel implements ListDataListener {
		...
		void updateColumnInfo() {
			...
			List<? extends RowSorter.SortKey> sortKeys = (rowSorter == null) ? null
		...
		private void restoreSortKeys(List<? extends RowSorter.SortKey> sortKeys) {
			List<? extends RowSorter.SortKey> keys = sortKeys;
			if(keys != null) {
		...
		public void setValueAt(Object value, int row, int col) {
			if(col == COLUMN_FILENAME) {
				...
				if(f != null) {
					String oldDisplayName = fileChooser.getName(f);
					...
					if(!newDisplayName.equals(oldDisplayName)) {
						...
						if(f2.exists()) {
							...
							if(directoryModel.renameFile(f, f2)) {
								if(fsv.isParent(fileChooser.getCurrentDirectory(),
										f2)) {
									// The setSelectedFile method produces a new
									// setValueAt invocation while the JTable
									// is editing. Postpone file selection to be
									// sure that edit mode of the JTable
									// is completed
									SwingUtilities.invokeLater(new Runnable() {
										public void run() {
											if(fileChooser.isMultiSelectionEnabled()) {
												fileChooser.setSelectedFiles(
														new File[] { f2 });
											} else {
								...
								JOptionPane.showMessageDialog(fileChooser,
										MessageFormat.format(renameErrorText,
												oldFileName),
										renameErrorTitleText,
										JOptionPane.ERROR_MESSAGE);
							}
		...
		public void contentsChanged(ListDataEvent e) {
			...
			SwingUtilities.invokeLater(new DelayedSelectionUpdater());
		...
		public void intervalAdded(ListDataEvent e) {
			...
			if(i0 == i1) {
				...
				if(file.equals(newFolderFile)) {
					SwingUtilities.invokeLater(new DelayedSelectionUpdater(file));
					newFolderFile = null;
				}
			}
	...
	/**
	 * This class sorts directories before files, comparing directory to directory and file to file using the
	 * wrapped comparator.
	 */
	...
	class DetailsTableCellRenderer extends DefaultTableCellRenderer {
		...
		public void setBounds(int x, int y, int width, int height) {
			...
			if(getHorizontalAlignment() == SwingConstants.LEADING && !fullRowSelection) {
				...
				w = Math.min(w, this.getPreferredSize().width + 4);
			} else {
				px -= 4;
			}
			super.setBounds(px, y, w, height);
		}
		...
		public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected,
			...
			if(value == null) {
				...
				/*
				 * This code block is relevant to Linux. File size is displayed up to 1 decimal
				 * precision. Base-10 number system is used for formatting file size similar to how it's
				 * formatted in file managers on Linux. Empty file size is shown as 0.0 KB, 1-100-byte
				 * files are shown as 0.1 KB, 101-200-byte files are shown as 0.2 KB and so on.
				 */
		...
		/**
		 * Rounds a value to one decimal place. It's used to format file size similar to how it's formatted in
		 * file managers on Linux. For example, the file size of 1200 bytes is rounded to 1.2 KB.
		 *
		 * @param fileSize
		 *                the file size to round to one decimal place
	...
	public JPanel createDetailsView() {
		...
		final JTable details = new JTable(getDetailsTableModel()) {
		...
		details.setRowSorter(getRowSorter());
		details.setAutoCreateColumnsFromModel(false);
		details.setComponentOrientation(chooser.getComponentOrientation());
		details.setAutoResizeMode(JTable.AUTO_RESIZE_OFF);
		details.setShowGrid(false);
		details.putClientProperty("JTable.autoStartsEdit", Boolean.FALSE);
		details.addKeyListener(detailsKeyListener);

		Font font = list.getFont();
		details.setFont(font);
		details.setIntercellSpacing(new Dimension(0, 0));

		TableCellRenderer headerRenderer = new AlignableTableHeaderRenderer(
				details.getTableHeader().getDefaultRenderer());
		details.getTableHeader().setDefaultRenderer(headerRenderer);
		TableCellRenderer cellRenderer = new DetailsTableCellRenderer(chooser);
		details.setDefaultRenderer(Object.class, cellRenderer);

		// So that drag can be started on a mouse press
		details.getColumnModel().getSelectionModel().setSelectionMode(ListSelectionModel.SINGLE_SELECTION);

		details.addMouseListener(getMouseHandler());
		// No need to addListSelectionListener because selections are forwarded
		// to our JList.

		// 4835633 : tell BasicTableUI that this is a file list
		details.putClientProperty("Table.isFileList", Boolean.TRUE);

		if(listViewWindowsStyle) {
			details.addFocusListener(repaintListener);
		}

		details.setFocusTraversalKeys(KeyboardFocusManager.FORWARD_TRAVERSAL_KEYS, null);
		details.setFocusTraversalKeys(KeyboardFocusManager.BACKWARD_TRAVERSAL_KEYS, null);
		...
		scrollpane.addMouseListener(new MouseAdapter() {
			...
			public void mousePressed(MouseEvent e) {
				...
				if(!e.isShiftDown() || table.getSelectionModel()
		...
		details.setForeground(list.getForeground());
		details.setBackground(list.getBackground());
		...
		details.putClientProperty(AccessibleContext.ACCESSIBLE_NAME_PROPERTY, filesDetailsAccessibleName);
	...
	private class AlignableTableHeaderRenderer implements TableCellRenderer {
		...
		public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected,
			...
			Component c = wrappedRenderer.getTableCellRendererComponent(table, value, isSelected, hasFocus,
					row, column);
			...
			if(!newDisplayName.equals(oldDisplayName)) {
				...
				if(f2.exists()) {
					JOptionPane.showMessageDialog(chooser,
							MessageFormat.format(renameErrorFileExistsText, oldFileName),
	...
	protected class FileRenderer extends DefaultListCellRenderer {
		@Override
		public Component getListCellRendererComponent(JList<?> lst, Object value, int index, boolean isSelected,
				boolean cellHasFocus) {
			boolean selected = isSelected;
			if(listViewWindowsStyle && !lst.isFocusOwner()) {
				selected = false;
			}

			super.getListCellRendererComponent(lst, value, index, selected, cellHasFocus);
			File file = (File) value;
	...
	/*
	 * Listen for filechooser property changes, such as the selected file changing, or the type of the dialog
	 * changing.
	 */
	...
	public void ensureFileIsVisible(@SuppressWarnings("unused") JFileChooser fc, File f) {
	...
	public JPopupMenu getComponentPopupMenu() {
		...
		if(contextMenu == null) {
			...
			Action newFolder = actionMap.get(ACTION_NEW_FOLDER);
			if(refreshAction != null) {
				contextMenu.add(refreshAction);
				if(listViewWindowsStyle && newFolder != null) {
					contextMenu.addSeparator();
				}
			}
			if(newFolder != null) {
		...
		if(view != null) {
			view.getPopupMenu().setInvoker(view);
		}
	...
	private class Handler implements MouseListener {
		...
		public void mouseClicked(MouseEvent e) {
			...
			if(source instanceof JList) {
				...
				boolean pointOutsidePrefSize = SwingUtilities2.pointOutsidePrefSize(
				...
				if(index >= 0 && list != null &&
					...
					Rectangle r = list.getCellBounds(index, index);
					MouseEvent newEvent = new MouseEvent(list, evt.getID(), evt.getWhen(),
							evt.getModifiers(), r.x + 1, r.y + r.height / 2,
							evt.getXOnScreen(), evt.getYOnScreen(), evt.getClickCount(),
