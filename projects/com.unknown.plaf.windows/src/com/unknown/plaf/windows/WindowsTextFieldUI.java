...
/**
 * Provides the Windows look and feel for a text field. This is basically the following customizations to the default
 * look-and-feel.
 * <ul>
 ...
 * <li>The foreground color is high contrast in the selected area, white by default. The unselected foreground is black.
...
public final class WindowsTextFieldUI extends BasicTextFieldUI {
	...
	/**
	 * Paints a background for the view. This will only be called if isOpaque() on the associated component is true.
	 * The default is to paint the background color of the component.
	 *
	 * @param g
	...
	static final class WindowsFieldCaret extends DefaultCaret implements UIResource {
		...
		/**
		 * Adjusts the visibility of the caret according to the windows feel which seems to be to move the caret
		 * out into the field by about a quarter of a field length if not visible.
		 */
		@Override
		...
		private final class SafeScroller implements Runnable {
			...
			public void run() {
				...
				if(field != null) {
					...
					int px = r.x + vis.getValue() - i.left;
					int quarterSpan = vis.getExtent() / 4;
					if(r.x < i.left) {
						vis.setValue(px - quarterSpan);
					} else if(r.x + r.width > i.left + vis.getExtent()) {
						vis.setValue(px - (3 * quarterSpan));
					}
