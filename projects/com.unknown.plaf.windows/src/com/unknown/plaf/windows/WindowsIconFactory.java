...
public final class WindowsIconFactory implements Serializable {
	...
	private static final class CheckBoxMenuItemIcon implements Icon, UIResource, Serializable {
		...
		public void paintIcon(Component c, Graphics g, int x, int y) {
			...
			if(isSelected) {
				int py = y;
				py = y - getIconHeight() / 2;
				g.drawLine(x + 9, py + 3, x + 9, py + 3);
				g.drawLine(x + 8, py + 4, x + 9, py + 4);
				g.drawLine(x + 7, py + 5, x + 9, py + 5);
				g.drawLine(x + 6, py + 6, x + 8, py + 6);
				g.drawLine(x + 3, py + 7, x + 7, py + 7);
				g.drawLine(x + 4, py + 8, x + 6, py + 8);
				g.drawLine(x + 5, py + 9, x + 5, py + 9);
				g.drawLine(x + 3, py + 5, x + 3, py + 5);
				g.drawLine(x + 3, py + 6, x + 4, py + 6);
			}
		}
	...
	private static final class MenuItemCheckIcon implements Icon, UIResource, Serializable {
		...
		public void paintIcon(Component c, Graphics g, int x, int y) {
			/*
			 * For debugging: Color oldColor = g.getColor(); g.setColor(Color.orange);
			 * g.fill3DRect(x,y,getIconWidth(), getIconHeight(), true); g.setColor(oldColor);
			 */
		}
	...
	private static final class MenuItemArrowIcon implements Icon, UIResource, Serializable {
		...
		public void paintIcon(Component c, Graphics g, int x, int y) {
			/*
			 * For debugging: Color oldColor = g.getColor(); g.setColor(Color.green);
			 * g.fill3DRect(x,y,getIconWidth(), getIconHeight(), true); g.setColor(oldColor);
			 */
		}
	...
	static final class VistaMenuItemCheckIconFactory implements MenuItemCheckIconFactory {
		...
		/**
		 * CheckIcon for JMenuItem, JMenu, JCheckBoxMenuItem and JRadioButtonMenuItem. Note: to be used on Vista
		 * only.
		 */
		private static final class VistaMenuItemCheckIcon implements Icon, UIResource, Serializable {
						...
						if(xp != null) {
							...
							if(WindowsGraphicsUtils.isLeftToRight(c)) {
								if(icon == null || icon.getIconHeight() <= 16) {
									...
									skin.paintSkin(g, x + OFFSET,
											y + icon.getIconHeight() / 2,
											state);
								}
							} else {
								if(icon == null) {
									skin.paintSkin(g, x + 4 * OFFSET, y + OFFSET,
											state);
								} else {
									...
									if(icon.getIconWidth() <= 8) {
										skin.paintSkin(g, x + OFFSET, ycoord,
												state);
			...
			private static boolean isEnabled(Component c, State state) {
				State st = state;
				if(st == null && c instanceof JMenuItem) {
					WindowsMenuItemUIAccessor accessor = getAccessor((JMenuItem) c);
					if(accessor != null) {
						st = accessor.getState((JMenuItem) c);
					}
				}
				if(st == null) {
					if(c != null) {
					...
					return (st != State.DISABLED) && (st != State.DISABLEDHOT) &&
							(st != State.DISABLEDPUSHED);
				}
			}
