...
/**
 * Windows implementation of RootPaneUI, there is one shared between all JRootPane instances.
 *
...
public final class WindowsRootPaneUI extends BasicRootPaneUI {
	...
	static final class AltProcessor implements KeyEventPostProcessor {
		...
		static void altPressed(KeyEvent ev) {
		...
		static void altReleased() {
			...
			if(msm.getSelectedPath().length == 0) {
				...
				// ev.getWhen() <= ((SunToolkit) tk)
				// .getWindowDeactivationTime(winAncestor);
				// }
	...
	/**
	 * Invokes supers implementation of <code>installUI</code> to install the necessary state onto the passed in
	 * <code>JRootPane</code> to render the metal look and feel implementation of <code>RootPaneUI</code>. If the
	 * <code>windowDecorationStyle</code> property of the <code>JRootPane</code> is other than
	 * <code>JRootPane.NONE</code>, this will add a custom <code>Component</code> to render the widgets to
	 * <code>JRootPane</code>, as well as installing a custom <code>Border</code> and <code>LayoutManager</code> on
	...
	/**
	 * Invokes supers implementation to uninstall any of its state. This will also reset the
	 * <code>LayoutManager</code> of the <code>JRootPane</code>. If a <code>Component</code> has been added to the
	 * <code>JRootPane</code> to render the window decoration style, this method will remove it. Similarly, this
	 * will revert the Border and LayoutManager of the <code>JRootPane</code> to what it was before
	...
	/**
	 * Installs the necessary state onto the JRootPane to render client decorations. This is ONLY invoked if the
	 * <code>JRootPane</code> has a decoration style other than <code>JRootPane.NONE</code>.
	 */
	private void installClientDecorations(JRootPane rootPane) {
		installBorder(rootPane);

		JComponent title = createTitlePane(rootPane);

		setTitlePane(rootPane, title);
	...
	private JComponent createTitlePane(JRootPane rootPane) {
		return new WindowsTitlePane(rootPane, this);
	}
	...
	/**
	 * Returns the <code>JComponent</code> rendering the title pane. If this returns null, it implies there is no
	...
	/**
	 * Invoked when a property changes. <code>WindowsRootPaneUI</code> is primarily interested in events originating
	 * from the <code>JRootPane</code> it has been installed on identifying the property
	 * <code>windowDecorationStyle</code>. If the <code>windowDecorationStyle</code> has changed to a value other
	 * than <code>JRootPane.NONE</code>, this will add a <code>Component</code> to the <code>JRootPane</code> to
	 * render the window decorations, as well as installing a <code>Border</code> on the <code>JRootPane</code>. On
	 * the other hand, if the <code>windowDecorationStyle</code> has changed to <code>JRootPane.NONE</code>, this
	 * will remove the <code>Component</code> that has been added to the <code>JRootPane</code> as well resetting
	 * the Border to what it was before <code>installUI</code> was invoked.
	 *
	 * @param e
	...
	public void propertyChange(PropertyChangeEvent e) {
		...
		if(propertyName.equals("windowDecorationStyle")) {
			...
			// simpler. WindowsTitlePane also assumes it will be recreated if
	...
	/**
	 * A custom layout manager that is responsible for the layout of layeredPane, glassPane, menuBar and titlePane,
	 * if one has been installed.
	 */
	// NOTE: Ideally this would extends JRootPane.RootLayout, but that
	// would force this to be non-static.
	private static class WindowsRootLayout implements LayoutManager2 {
		/**
		...
		public Dimension preferredLayoutSize(Container parent) {
			...
			if(root.getWindowDecorationStyle() != JRootPane.NONE &&
					(root.getUI() instanceof WindowsRootPaneUI)) {
				JComponent titlePane = ((WindowsRootPaneUI) root.getUI()).getTitlePane();
				if(titlePane != null) {
	...
	private class MouseInputHandler implements MouseInputListener {
		...
		/**
		 * Returns the Cursor to render for the specified corner. This returns 0 if the corner doesn't map to a
		 * valid Cursor
		 */
		 ...
		 * width - BORDER_DRAG_THICKNESS 3 if >= width - CORNER_DRAG_WIDTH 4 if >= width - BORDER_DRAG_THICKNESS
		 * 5 otherwise
		 */
