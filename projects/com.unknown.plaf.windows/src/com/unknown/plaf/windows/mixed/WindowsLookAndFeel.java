...
import com.unknown.plaf.windows.mixed.WindowsIconFactory.VistaMenuItemCheckIconFactory;
import com.unknown.plaf.windows.mixed.XPStyle.Skin;
import com.unknown.util.ui.plaf.DefaultLayoutStyle;
import com.unknown.util.ui.plaf.ImageIconUIResource;
import com.unknown.util.ui.plaf.MnemonicHandler;
import com.unknown.util.ui.plaf.OSInfo;
import com.unknown.util.ui.plaf.ShellFolder;
import com.unknown.util.ui.plaf.SortArrowIcon;
import com.unknown.util.ui.plaf.StringUIClientPropertyKey;
import com.unknown.util.ui.plaf.SunToolkit;
import com.unknown.util.ui.plaf.SwingUtilities2;

/**
 * Implements the Windows95/98/NT/2000 Look and Feel. UI classes not implemented specifically for Windows will default
 * to those implemented in Basic.
 */
@SuppressWarnings("serial") // Superclass is not serializable across versions
public class WindowsLookAndFeel extends BasicLookAndFeel {
	/**
	 * A client property that can be used with any JComponent that will end up calling the
	 * LookAndFeel.getDisabledIcon method. This client property, when set to Boolean.TRUE, will cause
	 * getDisabledIcon to use an alternate algorithm for creating disabled icons to produce icons that appear
	...
	@SuppressWarnings("unused") private WindowsDesktopProperty themeActive, dllName, colorName, sizeName;
	@SuppressWarnings("unused") private WindowsDesktopProperty aaSettings;
	...
	/**
	 * Initialize the uiClassID to BasicComponentUI mapping. The JComponent classes define their own uiClassID
	 * constants (see AbstractComponent.getUIClassID). This table must map those constants to a BasicComponentUI
	...
	protected void initClassDefaults(UIDefaults table) {
		...
		final String windowsPackageName = "com.unknown.plaf.windows.mixed.";
	 ...
	 * the public fields in SystemColor. If the table is being created on a native Windows platform we use the
	 * SystemColor values, otherwise we create color objects whose values match the defaults Windows95 colors.
	 */
	@Override
	protected void initSystemColorDefaults(UIDefaults table) {
		String[] defaultSystemColors = {
				...
				"inactiveCaptionText", "#C0C0C0", /*
									 * Text color for text in inactive captions
									 * (title bars).
									 */
	...
	private static void initResourceBundle(UIDefaults table) {
		...
		// "com.unknown.plaf.windows.resources.windows");
		table.addResourceBundle("com.unknown.plaf.windows.mixed.resources.windows");
	}
	...
	protected void initComponentDefaults(UIDefaults table) {
		...
		if(useSystemFontSettings) {
			...
			/*
			 * Put the desktop AA settings in the defaults. JComponent.setUI() retrieves this and makes it
			 * available as a client property on the JComponent. Use the same key name for both client
			 * property and UIDefaults. Also need to set up listeners for changes in these settings.
			 */
		...
		Object[] defaults = {
				...
				new XPColorValue(Part.LVP_LISTVIEW, null, Prop.FILLCOLOR,
						WindowBackgroundColor),
				...
				"ToolTip.background",
				new WindowsDesktopProperty("win.tooltip.backgroundColor", table.get("info")),
				"ToolTip.foreground",
				new WindowsDesktopProperty("win.tooltip.textColor", table.get("infoText")),
	...
	private static void initVistaComponentDefaults(UIDefaults table) {
		if(!isOnVista()) {
			return;
		}
		...
		/*
		 * acceleratorSelectionForeground color is the same as acceleratorForeground
		 */
		for(int i = 0, j = 0; i < menuClasses.length; i++) {
		...
		Object minimumTextOffset = new UIDefaults.ActiveValue() {
			@Override
			public Object createValue(UIDefaults tab) {
	...
	/**
	 * If we support loading of fonts from the desktop this will return a WindowsDesktopProperty representing the
	 * font. If the font can't be represented in the current encoding this will return null and turn off the use of
	 * system fonts.
	 */
	...
	//
	private static Object[] getLazyValueDefaults() {
		...
		Color highlight = (Color) Toolkit.getDefaultToolkit().getDesktopProperty("win.3d.highlightColor");

		Color shadow = (Color) Toolkit.getDefaultToolkit().getDesktopProperty("win.3d.shadowColor");
		...
		Object[] lazyDefaults = {
				...
				"TitledBorder.border", new UIDefaults.LazyValue() {
					public Object createValue(UIDefaults table) {
						return new BorderUIResource.EtchedBorderUIResource(highlight, shadow);
					}
				},
	...
	/**
	 * Gets the state of the flag which indicates if the old Windows look and feel should be rendered. This flag is
	 * used by the component UI delegates as a hint to determine which style the component should be rendered.
	 *
	 ...
	 * <code>JTextField</code> that has focus.
	 * </p>
	 * <p>
	 * If the user has enabled visual error indication on the desktop, this method will flash the caption bar of the
	 * active window. The user can also set the property awt.visualbell=true to achieve the same results.
	 * </p>
	 *
	 * @param component
	 *                Component the error occurred in, may be null indicating the error condition is not directly
	 ...
	 * <P>
	 * This Action contains the information and logic to render an auditory cue. The <code>Object</code> that is
	 * passed to this method contains the information needed to render the auditory cue. Normally, this
	 * <code>Object</code> is a <code>String</code> that points to a <code>Toolkit</code>
	 * <code>desktopProperty</code>. This <code>desktopProperty</code> is resolved by AWT and the Windows OS.
	 * <P>
	 * This <code>Action</code>'s <code>actionPerformed</code> method is fired by the <code>playSound</code> method.
	 *
	 * @return an Action which knows how to render the auditory cue for one particular system or user activity
	...
	/**
	 * Pass the name String to the super constructor. This is used later to identify the Action and decide whether
	 * to play it or not. Store the resource String. It is used to get the audio resource. In this case, the
	 * resource is a <code>Runnable</code> supplied by <code>Toolkit</code>. This <code>Runnable</code> is
	 * effectively a pointer down into the Win32 OS that knows how to play the right sound.
	 *
	 * @since 1.4
	 */
	private static final class AudioAction extends AbstractAction {
		...
		/**
		 * We use the String as the name of the Action and as a pointer to the underlying OSes audio resource.
		 */
	...
	/**
	 * Gets an <code>Icon</code> from the native libraries if available, otherwise gets it from an image resource
	 * file.
	 */
	...
	/**
	 * Gets an <code>Icon</code> from the native libraries if available. A desktop property is used to trigger
	 * reloading the icon when needed.
	 */
	...
	/**
	 * WindowsDesktopProperty for fonts. If a font with the name 'MS Sans Serif' is returned, it is mapped to
	...
	/**
	 * WindowsDesktopProperty for fonts that only gets sizes from the desktop, font name and style are passed into
	 * the constructor
	 */
	private static final class WindowsFontSizeProperty extends
		...
		protected Object configureValue(Object value) {
			Object result = value;
			if(result == null) {
				result = new FontUIResource(fontName, fontStyle, fontSize);
			} else if(result instanceof Integer) {
				result = new FontUIResource(fontName, fontStyle, ((Integer) result).intValue());
			}
			return result;
		}
	}
	...
	/**
	 * A value wrapper that actively retrieves values from xp or falls back to the classic value if not running XP
	 * styles.
	 */
	private static class XPValue implements UIDefaults.ActiveValue {
		...
		private static Object recursiveCreateValue(Object value, UIDefaults table) {
			Object result = value;
			if(result instanceof UIDefaults.LazyValue) {
				result = ((UIDefaults.LazyValue) result).createValue(table);
			}
			if(result instanceof UIDefaults.ActiveValue) {
				return ((UIDefaults.ActiveValue) result).createValue(table);
			} else {
				return result;
			}
		}
