...
import com.unknown.util.ui.plaf.DefaultTableCellHeaderRenderer;
...
public final class WindowsTableHeaderUI extends BasicTableHeaderUI {
	...
	private final class XPDefaultRenderer extends DefaultTableCellHeaderRenderer {
		...
		public Component getTableCellRendererComponent(JTable table, Object value, boolean selected,
			...
			this.hasRollover = (col == getRolloverColumn());
			if(skin == null) {
			...
			/*
			 * idk: Both on Vista and XP there is some offset to the HP_HEADERITEM content. It does not seem
			 * to come from Prop.CONTENTMARGINS. Do not know where it is defined. using some hardcoded
			 * values.
			 */
			contentLeft += 5;
			contentBottom += 4;
			...
			/*
			 * On Vista sortIcon is painted above the header's text. We use border to paint it.
			 */
			Icon sortIcon;
			if(WindowsLookAndFeel.isOnVista() &&
				...
				setIcon(null);
				sortIcon = null;
				SortOrder sortOrder = getColumnSortOrder(table, col);
				if(sortOrder != null) {
					switch(sortOrder) {
					case ASCENDING:
	...
	/**
	 * A border with an Icon at the middle of the top side. Outer insets can be provided for this border.
	 */
