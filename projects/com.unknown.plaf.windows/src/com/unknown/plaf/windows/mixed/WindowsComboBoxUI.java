...
import com.unknown.plaf.windows.mixed.WindowsBorders.DashedBorder;
import com.unknown.plaf.windows.mixed.XPStyle.Skin;
import com.unknown.util.ui.plaf.DefaultLookup;
...
public final class WindowsComboBoxUI extends BasicComboBoxUI {
	...
	protected void installListeners() {
		...
		if(xp != null && XPStyle.isSkinDefined(comboBox, Part.CP_DROPDOWNBUTTONRIGHT)) {
	...
	State getXPComboBoxState(JComponent c) {
		...
		if(!c.isEnabled()) {
			...
			state = State.PRESSED;
		} else if(comboBox.isEditable() && comboBox.getEditor().getEditorComponent().isFocusOwner()) {
			state = State.PRESSED;
		} else if(isRollover) {
			state = State.HOT;
		}
	...
	private void paintXPComboBoxBackground(Graphics g, JComponent c) {
		...
		Skin skin = null;
		if(!comboBox.isEditable() && XPStyle.isSkinDefined(c, Part.CP_READONLY)) {
	...
	public void paintCurrentValue(Graphics g, Rectangle bounds, boolean focus) {
		...
		if(!comboBox.isEditable() && xp != null && XPStyle.isSkinDefined(comboBox, Part.CP_READONLY)) {
			...
			Component c;
			if(focus && !isPopupVisible(comboBox)) {
			...
			if(comboBox.isEnabled()) {
				...
				c.setForeground(DefaultLookup.getColor(comboBox, this, "ComboBox.disabledForeground",
						null));
				c.setBackground(DefaultLookup.getColor(comboBox, this, "ComboBox.disabledBackground",
						null));
			}
	...
	public void paintCurrentValueBackground(Graphics g, Rectangle bounds, boolean focus) {
		if(XPStyle.getXP() == null) {
			super.paintCurrentValueBackground(g, bounds, focus);
		}
	}
			...
			public void layoutContainer(Container parent) {
				...
				if(XPStyle.getXP() != null && arrowButton != null) {
					...
					arrowButton.setBounds(
							WindowsGraphicsUtils.isLeftToRight(parent)
									? (d.width - (insets.right -
											borderInsetsCorrection) -
	...
	protected ListCellRenderer<Object> createRenderer() {
		XPStyle xp = XPStyle.getXP();
		if(xp != null && XPStyle.isSkinDefined(comboBox, Part.CP_READONLY)) {
	...
	private final class XPComboBoxButton extends XPStyle.GlyphButton {
		...
		public XPComboBoxButton() {
			super(comboBox,
					(!XPStyle.isSkinDefined(comboBox, Part.CP_DROPDOWNBUTTONRIGHT))
		...
		protected State getState() {
			...
			if(rv != State.DISABLED && comboBox != null && !comboBox.isEditable() && xp != null &&
					XPStyle.isSkinDefined(comboBox, Part.CP_DROPDOWNBUTTONRIGHT)) {
				/*
				 * for non editable ComboBoxes Vista seems to have the same glyph for all non DISABLED
				 * states
				 */
				rv = State.NORMAL;
			}
			if(rv == State.NORMAL && (prevState == State.HOT || prevState == State.PRESSED)) {
				/*
				 * State NORMAL of combobox button cannot overpaint states HOT or PRESSED Therefore HOT
				 * state must be painted from alpha 1 to 0 and not as usual that NORMAL state is painted
	...
	protected final class WinComboPopUp extends BasicComboPopup {
		...
		public WinComboPopUp(JComboBox<Object> combo) {
			super(combo);
			xp = XPStyle.getXP();
			if(xp != null && XPStyle.isSkinDefined(combo, Part.LBCP_BORDER_NOSCROLL)) {
	...
	/**
	 * Subclassed to set opacity {@code false} on the renderer and to show border for focused cells.
	 */
	@SuppressWarnings("serial") // Superclass is not serializable across versions
	private static final class WindowsComboBoxRenderer
		...
		public Component getListCellRendererComponent(
			...
			Component rv = super.getListCellRendererComponent(list, value, index,
					isSelected, cellHasFocus);
			if(rv instanceof JComponent) {
				JComponent component = (JComponent) rv;
				if(index == -1 && isSelected) {
					Border border = component.getBorder();
					Border dashedBorder = new WindowsComboBoxDashedBorder(list.getForeground());
