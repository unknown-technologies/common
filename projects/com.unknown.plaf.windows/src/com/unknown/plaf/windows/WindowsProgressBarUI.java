...
public final class WindowsProgressBarUI extends BasicProgressBarUI {
	...
	public int getBaseline(JComponent c, int width, int height) {
		...
		if(XPStyle.getXP() != null && progressBar.isStringPainted() &&
			...
			baseline = y + (h + metrics.getAscent() - metrics.getLeading() - metrics.getDescent()) / 2;
		}
		return baseline;
	}
	...
	protected void paintDeterminate(Graphics g, JComponent c) {
		...
		if(xp != null) {
			...
			if(progressBar.isStringPainted()) {
				...
				g2.setStroke(new BasicStroke(vertical ? barRectWidth : barRectHeight,
						BasicStroke.CAP_BUTT, BasicStroke.JOIN_BEVEL));
				if(!vertical) {
					if(isLeftToRight) {
						g2.drawLine(0, barRectHeight / 2, amountFull, barRectHeight / 2);
					} else {
				...
				int chunkSize = XPStyle.getInt(progressBar, Part.PP_PROGRESS, null,
						Prop.PROGRESSCHUNKSIZE, 2);
				int spaceSize = XPStyle.getInt(progressBar, Part.PP_PROGRESS, null,
				...
				for(int i = 0; i < nChunks; i++) {
					if(vertical) {
						skin.paintSkin(g, 3,
								barRectHeight - i * (chunkSize + spaceSize) -
										chunkSize - 2,
						...
						if(isLeftToRight) {
							...
							skin.paintSkin(g, barRectWidth -
									(2 + (i + 1) * (chunkSize + spaceSize)), 2,
	...
	protected Rectangle getBox(Rectangle r) {
		...
		if(xp != null) {
			...
			int gap = XPStyle.getInt(progressBar, Part.PP_PROGRESS, null, Prop.PROGRESSSPACESIZE, 0);
	...
	private Rectangle getFullChunkBounds(Rectangle box) {
		boolean vertical = (progressBar.getOrientation() == JProgressBar.VERTICAL);
		XPStyle xp = XPStyle.getXP();
		int gap = (xp != null) ? XPStyle.getInt(progressBar, Part.PP_PROGRESS, null, Prop.PROGRESSSPACESIZE, 0)
				: 0;
