...
public final class WindowsBorders {
	...
	public static Border getRootPaneBorder() {
		UIDefaults table = UIManager.getLookAndFeelDefaults();
		Border rootPaneBorder = new BorderUIResource.CompoundBorderUIResource(
				BorderFactory.createBevelBorder(BevelBorder.RAISED,
						table.getColor("InternalFrame.borderColor"),
						table.getColor("InternalFrame.borderHighlight"),
						table.getColor("InternalFrame.borderDarkShadow"),
	...
	public static Border getBottomLineBorder(Color color, int thickness) {
		return new BottomLineBorder(color, thickness);
	}
	...
	/**
	 * A dashed border that paints itself in the complementary color of the component's background color.
	 */
	@SuppressWarnings("serial") // Superclass is not serializable across versions
	...
	public static final class BottomLineBorder extends AbstractBorder implements UIResource {
		...
		public void paintBorder(Component c, Graphics g, int x, int y, int width, int height) {
			Color oldColor = g.getColor();
			g.setColor(color);
			int py = y + height - thickness;
			if(thickness == 1) {
				g.drawLine(x, py, x + width, py);
			} else {
				g.fillRect(x, py, width, thickness);
			}
			g.setColor(oldColor);
		}
