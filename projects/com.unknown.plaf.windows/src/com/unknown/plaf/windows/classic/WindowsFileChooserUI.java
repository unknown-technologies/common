...
public final class WindowsFileChooserUI extends BasicFileChooserUI {
	...
	private final class WindowsFileChooserUIAccessor implements FilePane.FileChooserUIAccessor {
		...
		public JPanel createList() {
			return WindowsFileChooserUI.this.createList();
		}
	...
	public void installComponents(JFileChooser fc) {
		...
		// CurrentDir ComboBox
		JComboBox<File> tmp2 = new JComboBox<>() {
			@Override
			public Dimension getMinimumSize() {
		...
		JButton tmp4 = new JButton(getApproveButtonText(fc)) {
			...
			public Dimension getMaximumSize() {
				return approveButton.getPreferredSize().width > cancelButton.getPreferredSize().width
						? approveButton.getPreferredSize()
	...
	private void updateUseShellFolder() {
		// Decide whether to use the ShellFolder class to populate shortcut panel and combobox.
		JFileChooser fc = getFileChooser();

		if(FilePane.usesShellFolder(fc)) {
			if(placesBar == null && !UIManager.getBoolean("FileChooser.noPlacesBar")) {
				placesBar = new WindowsPlacesBar(fc, false);
	...
	private static Integer getMnemonic(String key, Locale l) {
	...
	protected void installListeners(JFileChooser fc) {
		...
		ActionMap actionMap = newActionMap();
		SwingUtilities.replaceUIActionMap(fc, actionMap);
	}

	protected ActionMap newActionMap() {
	...
	/**
	 * Returns the preferred size of the specified <code>JFileChooser</code>. The preferred size is at least as
	 * large, in both height and width, as the preferred size recommended by the file chooser's layout manager.
	 *
	 * @param c
	 *                a <code>JFileChooser</code>
	...
	private void doApproveButtonMnemonicChanged() {
		approveButton.setMnemonic(getApproveButtonMnemonic(getFileChooser()));
	}

	private void doControlButtonsChanged() {
	...
	/**
	 * Property to remember whether a directory is currently selected in the UI. This is normally called by the UI
	 * on a selection event.
	 *
		...
		private void addItem(File directory) {
			...
			try {
				File sf = useShellFolder ? ShellFolder.getShellFolder(canonical)
						: canonical;
				File f = sf;
				ArrayList<File> path = new ArrayList<>(10);
				do {
					path.add(f);
	 ...
	 */
	@SuppressWarnings("serial") // Superclass is not serializable across versions
	protected final class FilterComboBoxModel extends AbstractListModel<FileFilter>
